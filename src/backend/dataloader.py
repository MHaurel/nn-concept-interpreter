...
class DataLoader:
    def __init__(self, path, model, thresh=None):
        ...
        if not os.path.exists(os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname)) or not os.path.exists(
                os.path.join('..', 'interpreter_data', 'activations', self.dirname)) or \
                (self.new_thresh is not None and self.new_thresh != self.thresh):

            if not os.path.exists(os.path.join('..', 'interpreter_data', 'activations', self.dirname)):
                os.makedirs(os.path.join('..', 'interpreter_data', 'activations', self.dirname))

            print(f"Writing in {os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname)}")
            ...
            for i in range(len(self.dfs)):
                pd.to_pickle(self.dfs[i], os.path.join('..', 'interpreter_data', 'activations', self.dirname,
                                                       f"{i} - {self.model.get_layers()[i].name}.pkl"))

            for i in range(len(self.non_standardized_dfs)):
                pd.to_pickle(self.non_standardized_dfs[i],
                             os.path.join('..', 'interpreter_data', 'activations', self.dirname,
            ...
            print(f"Using files in {os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname)}")
            # Doesn't compute data, only returns heatmaps
            for filename in os.listdir(os.path.join('..', 'interpreter_data', 'activations', self.dirname)):
                if filename.split('.')[-1] == 'pkl' and 'table_data' not in filename:
                    df = pd.read_pickle(os.path.join('..', 'interpreter_data', 'activations', self.dirname, filename))
    ...
    def get_popular_categories(self, thresh=500):
        ...
        with open(os.path.join('..', 'interpreter_data', 'activations', self.dirname, 'popular_categories.json'),
                  'w') as f:
    ...
    def get_all_activations(self, df, model, standardized=True):
        ...
        if isinstance(model.get_layers()[-1], Embedding):
            ...
            #df_fsr.to_pickle(os.path.join('..', 'interpreter_data', 'activations', self.dirname, "df_fsr.pkl"))
    ...
    def get_activation_for_cat(self, category, df, index):
        ...
        return_df = self.get_cat_df(category, df).loc[:, df.columns.isin(activations_cols)]
        if isinstance(self.model.get_layers()[index], Embedding):
            output_dim = self.model.get_layers()[index].output_shape[-1]
            arr = return_df.to_numpy().reshape(-1, output_dim)
            return self.get_mean_df_per_neuron(pd.DataFrame(arr))
        return return_df

    def get_mean_activation_for_cat(self, category, df, index):
        """
    ...
    def get_activation_for_not_cat(self, category, df, index):
        ...
        """
        activations_cols = [col for col in df.columns if "neuron" in col]
        return_df = self.get_not_cat_df(category, df).loc[:, df.columns.isin(activations_cols)]

        if isinstance(self.model.get_layers()[index], Embedding):
            output_dim = self.model.get_layers()[index].output_shape[-1]
            acts = []
            matrix = return_df.to_numpy()
            for i in range(len(matrix)):
                acts.append(pd.DataFrame(pd.DataFrame(matrix[i].reshape(-1, output_dim)).mean()).T)
            arr = return_df.to_numpy().reshape(-1, output_dim)
            return self.get_mean_df_per_neuron(pd.DataFrame(arr))
        return return_df
    ...
    def get_sample_for_cat(self, category, comparison_category, index=None, with_pv=False, misclassified=False):
        ...
        if not os.path.exists(current_path):
            ...
            for i in range(len(self.model.get_layers())):
                ...
                data_1 = self.get_activation_for_not_cat(category=category, df=self.dfs[i], index=i)
                ...
                if isinstance(self.model.get_layers()[i], Embedding):
                    sample_act = self.get_mean_df_per_neuron(pd.DataFrame(sample_act.to_numpy().reshape(-1, output_dim)))
    ...
    def get_pv_activation_for_sample(self, sample, category, df, index):
        """
        ...
        data_1 = self.get_activation_for_not_cat(category, df, index=index)
        data_1 = pd.DataFrame(data_1.mean()).T
    ...
    def get_sample_heatmaps_from_files(self, category, index):
        ...
        """
        dheatmaps = None
        file_path = os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname, 'sample',
    ...
    def find_pv(self, category, df, layer_name, i):
        """
        ...
        pvalue_dir = os.path.join('..', 'interpreter_data', 'pvalues', self.dirname, self.clean_s(category))
        ...
        if not os.path.exists(pvalue_path):
            ...
            actc = self.get_activation_for_cat(category, df, i)
            actnc = self.get_activation_for_not_cat(category, df, i)
            reses = []
    ...
    def get_heatmaps_dict(self):
        ...
        heatmap_path = os.path.join('..', 'interpreter_data', 'heatmaps')
        norm = matplotlib.colors.Normalize(-1, 1)
        colors = [[norm(-1.0), "cyan"],
                  [norm(-0.6), "lightblue"],
                  [norm(0.0), "black"],
        ...
        for i in range(len(self.model.get_layers())):#see with len(self.dfs)
            ddf = {}
            ...
            for c, n in self.get_popular_categories(self.thresh):
                ...
                plt.figure(figsize=(16, 5))
                data_1 = self.get_activation_for_not_cat(category=c, df=self.dfs[i], index=i)
                data_1 = pd.DataFrame(data_1.mean()).T

                diff = np.array(self.get_activation_for_cat(c, self.dfs[i], i).mean()) - np.array(
    ...
    def get_heatmaps_from_files(self):
        ...
        for dir in os.listdir(os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname)):
            if dir != 'sample':
                ddf = {}
                ...
                for p, n in self.get_popular_categories(self.thresh):
                    ...
                    hdiff = os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname, dir,
                                         f"{self.clean_s(p)}-diff.png")
                    heatmaps_dict["diff"] = {}
                    heatmaps_dict["diff"]['path'] = hdiff

                    hpv = os.path.join('..', 'interpreter_data', 'heatmaps', self.dirname, dir,
    ...
    def get_table_data(self):
        ...
        table_data_path = os.path.join('..', 'interpreter_data', 'activations', self.dirname, 'table_data.pkl')
